...
class CacheManager:
    ...
    def create_flag_file(self):
        ...
        try:
            with open(settings.CACHE_FILE, "w") as f:
                f.write(content)
            print(f"Successfully created cache flag file: '{settings.CACHE_FILE}'")
        except IOError as e:
            print(f"Error creating cache flag file '{settings.CACHE_FILE}': {e}")
    ...
    def create_content_cache(self):
        ...
        cache_parts = [
            ...
            for uri in settings.CACHE_OBJECTS_LIST
        ]
        content_cache = self.client.caches.create(
            model=settings.MODEL_NAME,
            config=CreateCachedContentConfig(
                contents=[Content(role="user", parts=cache_parts)],
                system_instruction="\n".join(settings.SYSTEM_INSTRUCTION),
                display_name=settings.CACHE_NAME,
    ...
    def get_cache_flag_file_data(self):
        ...
        try:
            ...
            print(f"Info: Cache file '{settings.CACHE_FILE}' not found.")
    ...
    def list_caches(self, cleanup=False):
        for content_cache in self.client.caches.list():
            ...
            # cleanup
            if content_cache.display_name != settings.CACHE_FILE:
                continue
